...
{
  SysTimer =  RTC->TSR*1000L ;
  SysTimer+=  RTC->TPR*1000L/32768L;
  return(SysTimer + t - 1);                             
}
...
*/
uint32_t GetSysMsTime(void)
{
  uint32_t time;
  time  = RTC->TSR*1000L;
  time += RTC->TPR*1000L/32768L;
  return time;
}

uint32_t GetSys100UsTime(void)
{
  uint32_t time;
  time  = RTC->TSR*10000L;
  time += RTC->TPR*10000L/32768L;
  return time;
}
