...
//!< API functions
void LPTMR_TC_Init(LPTMR_TC_InitTypeDef* LPTMR_TC_InitStruct);     //初始化配置LPTMR模块处于计时器模式
void LPTMR_PC_Init(LPTMR_PC_InitTypeDef* LPTMR_PC_InitStruct);	   //初始化配置LPTM模块处于脉冲计数模式
void LPTMR_ITDMAConfig(LPTMR_ITDMAConfig_Type config, bool status);//LPTM模块中断和DMA功能配置
void LPTMR_CallbackInstall(LPTMR_CallBackType AppCBFun);		   //注册LPTMR中断回调函数
uint32_t LPTMR_PC_ReadCounter(void);							   //获取脉冲计数器的脉冲数
uint32_t LPTMR_PC_QuickInit(uint32_t MAP);						   //LPTMR脉冲计数快速初始化
void LPTMR_ClearCounter(void);									   //清除脉冲计数器的脉冲数
