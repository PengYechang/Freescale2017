...
//!< API functions
void GPIO_Init(GPIO_InitTypeDef * GPIO_InitStruct);                     			//GPIO初始化配置,需要自己配置引脚
uint8_t GPIO_QuickInit(uint32_t instance, uint32_t pinx, GPIO_Mode_Type mode);      //GPIO快速初始化，不需要自己配置引脚
void GPIO_WriteBit(uint32_t instance, uint8_t pin, uint8_t data);					//设置指定引脚输出高电平或者低电平
uint8_t GPIO_ReadBit(uint32_t instance, uint8_t pin);							    //读取一个引脚上的电平状态
void GPIO_ToggleBit(uint32_t instance, uint8_t pin);								//翻转一个引脚的电平状态
void GPIO_SetBit(uint32_t instance, uint32_t pin);									//置位指定引脚的电平状态（即为高电平）
void GPIO_ResetBit(uint32_t instance, uint32_t pin);								//复位指定引脚的电平状态（即为低电平）
void GPIO_ITDMAConfig(uint32_t instance, uint8_t pin, GPIO_ITDMAConfig_Type config, bool status);//设置GPIO引脚中断类型或者DMA功能
void GPIO_CallbackInstall(uint32_t instance, GPIO_CallBackType AppCBFun);			//注册中断回调函数
//!< low level functions
...
uint32_t GPIO_ReadPort(uint32_t instance);										    //读取一个端口32位的数据
void GPIO_WritePort(uint32_t instance, uint32_t data);								//向一个端口写入32位数据
