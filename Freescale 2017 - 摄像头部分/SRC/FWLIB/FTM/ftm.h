...
/* PWM functions */
uint8_t FTM_PWM_QuickInit(uint32_t MAP, FTM_PWM_Mode_Type mode, uint32_t req);	    //快速配置初始化FTM模块实现PWM功能
void FTM_PWM_ChangeDuty(uint32_t instance, uint8_t chl, uint32_t pwmDuty);		    //更改指定引脚的PWM波形占空比
void FTM_PWM_InvertPolarity(uint32_t instance, uint8_t chl, uint32_t config);       //内部函数
/* QD functions */
uint32_t FTM_QD_QuickInit(uint32_t MAP, FTM_QD_PolarityMode_Type polarity, FTM_QD_Mode_Type mode); // 快速配置初始化FTM模块实现正交解码功能    
void FTM_QD_GetData(uint32_t instance, int* value, uint8_t* direction);			    //获得正交解码的数据     
void FTM_QD_ClearCount(uint32_t instance);											//复位FTM模块的计数值（清零）   
/* IC functions */
void FTM_IC_QuickInit(uint32_t MAP, FTM_ClockDiv_Type ps);							//初始化FTM 输入捕捉功能
void FTM_IC_SetTriggerMode(uint32_t instance, uint32_t chl, FTM_IC_Mode_Type mode); //设置输入捕捉触发模式
/* IT & DMA config */
void FTM_ITDMAConfig(uint32_t instance, FTM_ITDMAConfig_Type config, bool flag);    //FTM中断DMA控制
void FTM_CallbackInstall(uint32_t instance, FTM_CallBackType AppCBFun);				//设置FTM回调函数
/* control function */
uint32_t FTM_GetChlCounter(uint32_t instance, uint32_t chl);						//获得FTM通道计数值
void FTM_SetMoudleCounter(uint32_t instance, uint32_t val);							//设置FTM主计数Counter值
