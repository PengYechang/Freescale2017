...
/* API functions */
uint32_t DMA_ChlAlloc(void);											//DMA通道分配
void DMA_ChlFree(uint32_t chl);											//DMA通道释放
uint32_t DMA_Init(DMA_InitTypeDef *DMA_InitStruct);			   			//初始化DMA模块
/* get vars and status */
uint32_t DMA_GetDestAddress(uint8_t ch);								//获取DMA模块指定通道的目标地址
uint32_t DMA_GetSourceAddress(uint8_t ch);								//获取DMA模块指定通道的源地址
uint8_t DMA_IsMajorLoopComplete(uint8_t chl);							//检测DMA传输是否完成
uint32_t DMA_GetMajorLoopCount(uint8_t chl);							//获得DMA主循环计数值
/* control */
void DMA_SetMajorLoopCounter(uint8_t chl, uint32_t val);				//设置DMA主循环计数值
void DMA_CancelTransfer(void);											//取消DMA模块指定通道的数据传输
void DMA_SetDestAddress(uint8_t chl, uint32_t address);					//设置DMA模块指定通道的目标地址
void DMA_SetSourceAddress(uint8_t chl, uint32_t address);				//设置DMA模块指定通道的源地址
void DMA_EnableRequest(uint8_t chl);									//使能通道响应传输
void DMA_DisableRequest(uint8_t chl);									//禁止通道响应传输
void DMA_EnableAutoDisableRequest(uint8_t chl , bool flag);				//在主循环结束后，是否自动关闭Request
/* IT funtions */			
void DMA_ITConfig(uint8_t chl, DMA_ITConfig_Type config, bool status);	//设置DMA传输完成中断
void DMA_CallbackInstall(uint8_t chl, DMA_CallBackType AppCBFun);		//注册中断回调函数
/* chl link */
void DMA_EnableMajorLink(uint8_t chl , uint8_t linkChl, bool flag);		//使能Major LoopLink 功能
