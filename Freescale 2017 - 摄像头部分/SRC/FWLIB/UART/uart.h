...
/* API functions */
uint8_t UART_QuickInit(uint32_t MAP, uint32_t baudrate);       //UART快速初始化，返回的是UART硬件模块号
void UART_Init(UART_InitTypeDef * UART_InitStruct);            //UART初始化，需要自己进行引脚初始化
void UART_DeInit(uint32_t instance);						   //禁止某一个UART硬件模块号
int UART_printf(uint32_t instance, const char *format,...);    //UART打印函数，类似stdio.h中的printf
uint8_t UART_ReadByte(uint32_t instance, uint16_t *ch);        //读一个字节，接收之后立即返回，无阻塞
void UART_WriteByte(uint32_t instance, uint16_t ch);           //写一个字节，发送之后需等之前的发送完毕，有阻塞
void UART_SelectDebugInstance(uint32_t instance);              //UARTx当做调试端口来使用

/* FIFO functions */
void UART_EnableTxFIFO(uint32_t instance, bool status);        //使能UART的发送FIFO功能
void UART_EnableRxFIFO(uint32_t instance, bool status);        //使能UART的接收FIFO功能
uint32_t UART_GetTxFIFOSize(uint32_t instance);				   //获取UART发送中FIFO的通道深度
uint32_t UART_GetRxFIFOSize(uint32_t instance);                //获取UART接收中FIFO的通道深度
void UART_SetTxFIFOWatermark(uint32_t instance, uint32_t size);//设定发送FIFO通道的水位
void UART_SetRxFIFOWatermark(uint32_t instance, uint32_t size);//设定接收FIFO通道的水位

/* Interrupt and DMA functions */
void UART_CallbackTxInstall(uint32_t instance, UART_CallBackTxType AppCBFun); //注册发送中断回调函数
void UART_CallbackRxInstall(uint32_t instance, UART_CallBackRxType AppCBFun); //注册接收中断回调函数
void UART_ITDMAConfig(uint32_t instance, UART_ITDMAConfig_Type config, bool status); //配置UART模块的中断或DMA属性
