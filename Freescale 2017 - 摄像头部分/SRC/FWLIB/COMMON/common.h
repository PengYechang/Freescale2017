...
/* configuration */
#if !defined(UART_DMA_SUPPORT)   //使用DMA传输串口的数据
...
#define EnableInterrupts    __enable_irq         //开启所有中断
#define DisableInterrupts   __disable_irq        //禁止所有中断
void DelayMs(uint32_t ms);                   	 //Ms延时
void DelayUs(uint32_t us);                   	 //Us延时
void DelayInit(void);                        	 //延时初始化
uint32_t GetClock(Clock_t clockName);        	 //获得系统各个总线时钟的频率
uint32_t QuickInitEncode(map_t * type);          //无需调用
void QuickInitDecode(uint32_t map, map_t* type); //无需调用
void EnterSTOPMode(bool enSleepOnExit);      	 //进入低功耗模式
uint32_t GetUID(void);                        	 //获得芯片UID信息(全球唯一识别码)

void DWT_DelayInit(void);                        //DWT延时初始化，无需调用
void DWT_DelayUs(uint32_t us);                   //DWT的US级延时，无需调用
void DWT_DelayMs(uint32_t ms);                   //DWT的MS级延时，无需调用

void NVIC_Init(IRQn_Type IRQn,uint32_t PriorityGroup,uint32_t PreemptPriority,uint32_t SubPriority);  //优先级配置
#ifdef __cplusplus
}
#endif
